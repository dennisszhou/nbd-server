use nbd_control_plane::CatalogUrl;
...
#[tokio::test]
async fn installed_nbd_server_doctor_runs_by_name_outside_source_tree() {
    let runtime = TestRuntime::new().expect("test runtime");
    migrate_catalog(&runtime).await;
    let cwd = TempRoot::new();
    assert_outside_repo(cwd.path());

    let output = Command::new("nbd-server")
        .env("PATH", path_with_binary(env!("CARGO_BIN_EXE_nbd-server")))
        ...
        .arg("--json")
        .output()
        .expect("run nbd-server by command name");
...
impl TempRoot {
    fn new() -> Self {
        ...
        let path = env::temp_dir().join(format!(
            "nbd-server-installed-smoke-{}-{unique}-{counter}",
            std::process::id()
        ));
